  ...
  try {
    ...
    await db.query(
      `UPDATE users SET 
        ...
        total_co2_saved_kg = total_co2_saved_kg + $3,
        last_active = NOW()
       WHERE id = $4`,
      [result.points || 0, result.distance_km || 0, result.co2_kg || 0, user.id]
    );
...
export const getRedemptions = onRequest(async (req: Request, res: Response) => {
  ...
  try {
    ...
    // Create a mock request with user ID header for getRedemptionHistory
    const mockReq = {
      headers: { 'x-user-id': user.id }
    };
    
    const result = await getRedemptionHistory(mockReq);
 ...
 * Get Daily Stats
 */
export const getDailyStats = onRequest(async (req: Request, res: Response) => {
  res.set('Access-Control-Allow-Origin', '*');
  ...
  try {
    ...
    const { getDailyStats: getDailyStatsService } = await import('./services/database');
    const stats = await getDailyStatsService(startDate, endDate);
 ...
 * Get Mode Stats
 */
export const getModeStats = onRequest(async (req: Request, res: Response) => {
  res.set('Access-Control-Allow-Origin', '*');
  res.set('Access-Control-Allow-Methods', 'GET, OPTIONS');
  res.set('Access-Control-Allow-Headers', 'Content-Type');
  ...
  try {
    const { getModeStats: getModeStatsService } = await import('./services/database');
    const stats = await getModeStatsService();
 ...
 * Get Leaderboard
 */
export const getLeaderboard = onRequest(async (req: Request, res: Response) => {
  res.set('Access-Control-Allow-Origin', '*');
  res.set('Access-Control-Allow-Methods', 'GET, OPTIONS');
  res.set('Access-Control-Allow-Headers', 'Content-Type');
  ...
  try {
    ...
    const { getLeaderboard: getLeaderboardService } = await import('./services/database');
